...
import { cantonmentSchema } from "@/schema/cantonment.schema";
...
const CantonmentCreationForm = ({ operation }: any) => {
  ...
  const form = useForm({
    resolver: zodResolver(cantonmentSchema),
    defaultValues: {
      name: "",
    },
  });
  ...
  useEffect(() => {
    if (operation === "update") {
      form.reset({
        name: cantonment?.name || "",
      });
      form.setValue("name", cantonment?.name || "");
    }
  }, [operation, selectedId, cantonment, form]);

  const createMutation = useMutation({
    mutationFn: createCantonment,
    onSuccess: () => {
      toast.success("Cantonment Create Successful");
      query.invalidateQueries({ queryKey: ["cantonment"] });
      closeDrawer();
    },
  ...
  const updateMutation = useMutation({
    mutationFn: updateCantonment,
    onSuccess: () => {
      toast.success("Cantonment Update Successful");
      query.invalidateQueries({ queryKey: ["cantonment"] });
      closeDrawer();
    },
    onError: (error: unknown) => {
  ...
  return (
    <React.Fragment>
      <DrawerHeader className="gap-1">
        <DrawerTitle>
          {operation === "update" ? "Update Cantonment" : "Create Cantonment"}
        </DrawerTitle>
        <DrawerDescription>
          Fill up the details below to{" "}
          {operation === "update" ? "update" : "create"} Cantonment.
        </DrawerDescription>
      </DrawerHeader>
...
export default CantonmentCreationForm;
